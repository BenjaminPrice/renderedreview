...
const fake = (text: string) => ({ tree: { type: "root", children: [] }, nodes: [], text }) as unknown as RenderedMarkdown;
...
it("renders larger documents in the Worker, showing progress until done, then caches them per blob", async () => {
  ...
  expect(calls[0]!.job).toEqual({ source, pr: { host: "github.com", owner: "o", repo: "r", number: 7 }, sha: "h", path: "doc.md" });
...
it("falls back to the main thread when the Worker cannot render", async () => {
  ...
  expect(result.current.rendering).toBe(false);
  expect(result.current.result).toMatchObject({ nodes: expect.arrayContaining([expect.objectContaining({ type: "heading", text: "Fallback" })]) });
});
