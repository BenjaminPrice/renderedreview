...
const github = (url?: string) =>
  loadConfig({ HOSTING_MODE: "community", ACCESS_POLICY: "disabled", GITHUB_URL: url }).github;
...
test("github.com policy blocks inline scripts and unapproved origins", () => {
  const csp = directives(contentSecurityPolicy(github(), "abc"));
  ...
  expect(Object.values(csp).flat().join(" ")).not.toMatch(/unsafe|data:|\*(?!\.)|https?:(?!\/\/)/);
});

test("Enterprise Server policy allows its own host instead of github.com", () => {
  const csp = directives(contentSecurityPolicy(github("https://ghe.example.com"), "abc"));
  expect(csp["img-src"]).toEqual(["'self'", "https://ghe.example.com", "https://*.ghe.example.com"]);
  expect(csp["connect-src"]).toEqual(["'self'", "https://ghe.example.com", "https://*.ghe.example.com"]);
});
