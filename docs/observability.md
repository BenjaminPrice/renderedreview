...
| `billing.contributor` | Recording an active private contributor fails (the publish still succeeds)  | `category` (`failed`), `error`                                                                                                                                                                                         |
| `rate.limited`        | A request is refused for exceeding one of the app's limits (it answers 429) | `category`: the limit (`guest`, `auth`, `writes`, `trial-start-user`, `trial-start-network`). A refused sign-in callback redirects (302) instead of answering 429. Never the client address, its hash or the user      |
...
- Client IP addresses, or the keyed hashes rate limits count them under.
...
| Rate limiting       | `rate.limited` by `category`                                                                                                 | Floods, abuse, or limits set too low for real use             |
...
| Platform errors     | `db.migration_failed`, `background.failed`, `billing.contributor`, and Cloudflare exceptions/CPU limits                      | Hosting platform and database                                 |
