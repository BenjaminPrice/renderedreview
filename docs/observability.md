...
| `rate.limited`        | A request is refused for exceeding one of the app's limits (it answers 429) | `category`: the limit (`guest`, `auth`, `writes`, `trial-start`). Never the client address, its hash or the user                                                                                                       |
